...
A descoberta automática de produtos usa a cascata documentada nos [ADR-034](docs/ADR/ADR-034-descoberta-produtos-sem-api-oficial.md) e [ADR-035](docs/ADR/ADR-035-amazon-shorts-associados.md). O ranking Amazon do Trends MCP é o sinal primário para Shorts; SocialCrawl BR e Tavily/Hacker News são fallbacks. Todas as fontes alimentam somente candidatos em `idea_queue`: não preenchem link afiliado nem confirmam comissão. Para monetizar, o operador valida manualmente o produto e cola no painel o link da plataforma correta. O [ADR-036](docs/ADR/ADR-036-monetizacao-afiliado-link-por-plataforma.md) define `affiliate_links.youtube` para Amazon/Shopee/Hotmart e `affiliate_links.tiktok` para o TikTok Shop, sem reutilização entre destinos. O TikTok Shop afiliado permanece desativado até a conta de criador atingir o requisito de seguidores; o fluxo não depende de CNPJ, loja ou estoque.
