...
export function makeResearchEvidence(
  research: ResearchData,
): Extract<ResearchEvidence, { provider: "gemini_google_search" }> {
  const text = JSON.stringify(research);
  const encoder = new TextEncoder();
  let offset = 0;
  return {
    version: "1.0.0", provider: "gemini_google_search", model: "gemini-3.6-flash",
