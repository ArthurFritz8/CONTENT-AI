...
11. **Emenda operacional ao ADR-004 após CI real:** a run34659806620 comprovou ausência de FFmpeg/ffprobe no `ubuntu-latest` atual. `scripts/setup-ffmpeg.sh` verifica os executáveis e instala o pacote gratuito da distribuição quando necessário; usado em CI, assets e render. A premissa de binário pré-instalado deixou de ser requisito oculto.
