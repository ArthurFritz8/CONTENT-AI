...
- Antes de existir sessão persistida, uma nova reserva pode atualizar a configuração e a revisão para permitir correções de preflight. Nenhuma mídia pode ter sido transmitida nessa fase. Depois do checkpoint, sessão, metadados, revisão e arquivo permanecem fixos; o contador diário nunca é zerado pela correção.
