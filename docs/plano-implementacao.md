...
- Migrations históricas **preservadas**, mais `20260911000000_audit_hardening.sql` e `20260911010000_research_evidence.sql`. Esta última adiciona `episodes.research_evidence`; aplicar antes de implantar os workers ADR-017. Pesquisas antigas sem snapshot autêntico ficam bloqueadas.
...
Após ADR-017, permanecem bloqueios **de implementação** (QA audiovisual/gate/publish/analytics/retention e exibição das evidências) e **externos** (cloud, credenciais, quotas, qualidade factual e validação real do grounding). QA determinístico e citações estão implementados e revalidados antes de assets. O comando `node scripts/preflight.mjs --production` deve continuar falhando enquanto os módulos obrigatórios não existirem. Não “resolver” isso trocando módulos obrigatórios por opcionais ou preenchendo scores de aprovação fictícios.
