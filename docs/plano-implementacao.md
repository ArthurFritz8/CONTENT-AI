...
4. **Fact-check:** padrões configurados em `system_config.fact_check` e vínculo literal com pesquisa implementados no ADR-016, com gramática segura de frases/`\m/\M` e relatórios auditáveis. Ainda falta preservar e verificar grounding/evidências reais e avaliar cobertura semântica das afirmações. O relatório sempre exige revisão humana; `qa_passed` com escopo determinístico não autoriza publicação.
...
Após ADR-016, permanecem bloqueios **de implementação** (grounding verificável, QA audiovisual/gate/publish/analytics/retention) e **externos** (cloud, credenciais, quotas e validação real). QA determinístico está implementado e revalidado antes de assets. O comando `node scripts/preflight.mjs --production` deve continuar falhando enquanto os módulos obrigatórios não existirem. Não “resolver” isso trocando módulos obrigatórios por opcionais ou preenchendo scores de aprovação fictícios.
