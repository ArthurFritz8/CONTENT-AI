...
Após ADR-021, cloud, credenciais essenciais, Telegram e agendamento estão configurados com o pipeline pausado. Permanecem bloqueios **de implementação** (QA audiovisual/publicação pública e Shorts/analytics/retention e apresentação dedicada do grounding) e **de validação real** (uma geração completa, qualidade factual, render real e upload privado). Gate humano foi validado no bot e em testes automatizados; decisão sobre um vídeo real ainda depende da primeira execução. O comando `node scripts/preflight.mjs --production` deve continuar falhando enquanto os módulos obrigatórios não existirem. Não “resolver” isso trocando módulos obrigatórios por opcionais ou preenchendo scores de aprovação fictícios.
