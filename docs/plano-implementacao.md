...
| 2 | `generate-research` | 0–1 | Implementado, precisa endurecer evidência | Persistir grounding metadata, validar URLs/claims contra evidências e regras factuais. Falta de grounding não pode virar “pesquisa verificada” por texto gerado sem busca. |
| 3 | `generate-script` + QA determinístico compartilhado | 2, contrato Zod | Implementado, QA pendente | Repair de no máximo1, inclusive JSON malformado; sources vinculadas à pesquisa; CTA com disclosure narrado; full_text coerente; sem claims proibidos. `category` mapeável a categoryId. |
...
| 7 | `telegram-bot` + gate QA/revisão | 3,5,6 | **Não implementado** | Webhook com secret_token, allowlist chat/user, dedup update_id, callback idempotente vinculado ao hash do render. Mostrar vídeo, fontes, licenças e alertas. Aprovação registra identidade/data; rejeição invalida autorização anterior. |
| 8 | Ledger de publicação + `publish-youtube` | 0,5,7 | **Não implementado** | Unique por episódio/plataforma/**variante**; lock antes de upload; upload retomável, refresh OAuth, confirmação/reconciliação de resultado ambíguo. Somente upload privado inicial aprovado; flags e descrição corretas. |
...
1. **Aprovação do objeto exato:** associar aprovação ao render revision e ao hash de conteúdo/metadata. Qualquer alteração depois da aprovação invalida o consentimento. Campos já existentes de usuário/data não substituem essa associação.
...
4. **Fact-check:** implementar padrões configurados em `system_config.fact_check`, convertendo conscientemente boundaries PostgreSQL `\m/\M` quando usados em JavaScript; regex não prova veracidade. Conferir evidências e registrar alertas para decisão humana. Nunca criar um “QA aprovado” default.
...
- Migration inicial e fila **preservadas**, mais `20260911000000_audit_hardening.sql`.
