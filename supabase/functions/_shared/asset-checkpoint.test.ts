...
for (const growth of [false, true]) Deno.test(`assets retoma áudio e só promove com todas as legendas (growth=${growth})`, async () => {
  ...
  const audioCount = growth ? 4 : 3;
  const episode: Record<string, unknown> = {id,status:"script",script_json:platformScript,product_image_url:null,tts_engine:"gemini",
  ...
  try {
    ...
    assertEquals(ttsCalls,audioCount);
    assertEquals(reservations,audioCount);
    assertEquals(assets.filter(a=>a.type==='subtitle').length,audioCount * 2);
  } finally {
